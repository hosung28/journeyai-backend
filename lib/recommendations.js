 ...
 * and Restaurants land in Phase 2) for a stay based on the trip-wide
 * preferences the user set on the Trip Preferences screen.
 *
 * Claude is also asked to include 1-2 city-signature picks per stay even
 ...
 * the same stay should not re-bill Anthropic. 30-min TTL matches the
...
function buildPlacesPrompt({ city, nights, travelers, prefs, alreadyPicked, count }) {
  ...
  return `Plan PLACES to visit for a ${nights}-night stay in ${city} (${travelers} traveler${travelers > 1 ? "s" : ""}).
 ...
 * (stay.places.some(x => x.id === place.id)) survive cache evictions.
...
async function recommendForStay({ city, nights, travelers, tripPreferences, alreadyPicked }) {
...
module.exports = { recommendForStay };
