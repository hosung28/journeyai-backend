...
function normalize(option, index) {
  ...
  // Force Recommended off for Flight regardless of source. Real flights are
  // already neutralised in realFlightOption; this catches the fallback path
  // where AeroAPI returned no data and Claude provided estimate Flight
  // options with recommended: true. Train / ferry / bus / drive / walk keep
  // Claude's flag — for those modes Claude's pick reflects "most practical
  // mode for the route" (e.g. bullet train Tokyo->Kyoto) and is a useful
  // signal, not an opinion about a specific option.
  const isFlight = /flight/i.test(mode);
  return {
    ...
    notes: option.notes || "",
    recommended: isFlight ? false : Boolean(option.recommended),
...
async function getTransportOptions({ from, to, departureDate, adults }) {
  ...
  if (realSchedules.length === 0) {
    // No real schedule data. Claude flight estimates are misleading without
    // real schedules + fares, so on the Flight card specifically: drop the
    // fake price, replace the notes line with an honest "search Google
    // Flights" prompt, and attach the bookingUrl deep-link so the user has
    // a real escape hatch. Non-flight modes (train, ferry, bus, drive,
    // walk) pass through unchanged — Claude's estimate for those is the
    // best we'll ever get short of partnering with each operator.
    const fallbackBookingUrl = buildGoogleFlightsUrl(from, to, departureDate);
    options = claudeOptions.map((o) =>
      /flight/i.test(o.mode)
        ? {
            ...o,
            priceFrom: 0,
            priceLabel: "",
            notes: "Live schedule unavailable — search on Google Flights",
