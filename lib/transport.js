...
function pickRecommended(offers) {
  ...
  /** Score in absolute dollars — see header. */
  const scoreOf = (o) => {
    const price = o.priceFrom > 0 ? o.priceFrom : 0;
    return price + (o.stops || 0) * 300;
  };
  let bestIdx = 0;
  ...
  if (isCheapest && winner.stops === 0) reason = "Cheapest · Nonstop";
    ...
    const extra = winner.priceFrom - cheapest;
    reason =
      extra > 0
        ? `Nonstop · only $${extra.toLocaleString()} more than cheapest`
        : "Nonstop · best value";
