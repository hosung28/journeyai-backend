...
function buildPrompt(destination, customEvents) {
  ...
  const departureLine = destination.departureTime
    ? `\nLast day DEPARTURE: traveler's flight departs at ${destination.departureTime}. The plan for the LAST day MUST end with these two fixed items (do not omit them, do not move them):
  1) kind:"place", name:"Transit to Airport", area:"${destination.hotel || "City Center"} → Airport", time: ~2.5 h before departure, note:"Allow extra time for traffic", travelFromPrev: realistic mode to airport
